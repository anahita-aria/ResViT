...
wa=""
dsname=""
parser = optparse.OptionParser("Train ResViT model.")
parser.add_option("-e", "--epoch", type=int, dest='epoch', help='Number of epochs used for training the ResViT model.')
...
def train_gpu(model, criterion, optimizer, scheduler, num_epochs, min_val_loss):
    ...
    plt.savefig( 'ResVit_Accu_Deepfakes.png' )
    ...
    plt.xlabel('epoch')
    plt.legend(['Train', 'Validation'], loc='upper left')
    plt.savefig( 'ResViT_Loss_Deepfakes.png' )
    
    return train_loss,train_accu,val_loss,val_accu, min_loss

def test(model):
    print('Test mode resnet50')
    with torch.no_grad():
        Sum = 0
